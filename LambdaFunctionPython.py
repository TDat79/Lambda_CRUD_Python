...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET' and path == status_check_path:
            response = build_response(200, 'Now proceed to manage user information')
        elif http_method == 'GET' and path == userURL:
            user_id = event['queryStringParameters']['userid']
            response = get_user(user_id)
        elif http_method == 'GET' and path == all_user:
            response = get_users()
        elif http_method == 'POST' and path == userURL:
            response = save_user(json.loads(event['body']))
        elif http_method == 'PATCH' and path == userURL:
            body = json.loads(event['body'])
            response = modify_user(body['userId'], body['updateKey'], body['updateValue'])
        elif http_method == 'DELETE' and path == userURL:
            body = json.loads(event['body'])
